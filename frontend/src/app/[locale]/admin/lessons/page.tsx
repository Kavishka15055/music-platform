...
export default function AdminLessonsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this lesson?')) {
      try {
        const res = await fetch(`http://localhost:3005/api/v1/lessons/${id}`, { method: 'DELETE' });
        if (res.ok) {
          fetchLessons();
        } else {
          console.error('Failed to delete lesson:', res.status);
          alert(`Failed to delete lesson: ${res.status}`);
        }
      } catch (error) {
  ...
  const handleStartLesson = async (id: string) => {
    try {
      const res = await fetch(`http://localhost:3005/api/v1/lessons/${id}/start`, { method: 'POST' });
      if (res.ok) {
        fetchLessons();
      } else {
  ...
  const handleEndLesson = async (id: string) => {
    try {
      const res = await fetch(`http://localhost:3005/api/v1/lessons/${id}/end`, { method: 'POST' });
      if (res.ok) {
        fetchLessons();
      } else {
        const errorData = await res.json().catch(() => ({}));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json().catch(() => ({}));
        console.error('Failed to save lesson:', res.status, errorData);
        alert(`Failed to save lesson: ${res.status} ${JSON.stringify(errorData)}`);
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-slate-800">Manage Live Lessons</h1>
        <button
          ...
          <tbody className="divide-y divide-gray-200">
            {lessons.map((lesson) => (
              <tr key={lesson.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 text-gray-600">{lesson.instructor}</td>
                <td className="px-6 py-4">
